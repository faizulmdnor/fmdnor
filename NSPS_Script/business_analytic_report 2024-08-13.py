...
from fleet_performance.core.configs.paths import DATA_DIRECTORY
...
DEVELOPER_EMAIL_LIST = [
    ...
    'faiz.shaari@novasourcepower.com',
    'venkata.ratnam@novasourcepower.com',
    'ahmad.zulikmal@novasourcepower.com'
]
from_email = 'BusinessAnalytics@novasourcepower.com'

fpp_init_path: Path = Path(fleet_performance.__file__)
fpp_home_directory: Path = fpp_init_path.parents[1]
nsps_png = ''
if os.path.isfile(os.path.join(fpp_home_directory, 'docs\\source\\images', 'novasource_logo.png')):
    nsps_png = os.path.join(fpp_home_directory, 'docs\\source\\images', 'novasource_logo.png')
else:
    nsps_png = os.path.join(fpp_home_directory, 'fleet_performance\\package_data\\images', 'novasource_logo.png')
...
def process_work_orders_for_prioritization_reports(work_orders, cmms, fleet_metadata, start,
    ...
    def cal_approx_lost_mwh(row):
        sun_hours_per_day = 8
        # Check if kwOffline is null or zero
        condition_null_or_empty = row['capacityOffline'] == 0 or pd.isnull(row['capacityOffline'])

        # Calculate the approximate lost MWh if kwOffline is available
        if not condition_null_or_empty:
            calculated_value = row['capacityOffline'] / 1000 * sun_hours_per_day
        else:
            calculated_value = row['lost_MWh']

        # Use the maximum of lostMWh and the calculated value
        return max(row['lost_MWh'], calculated_value)
    ...
    def assign_target_complete_date(row):
        if row['category'] == '3':
            target_complete_date = parser.parse(row['suggestedMaintenanceDate'] if row[
                                                                                       'suggestedMaintenanceDate'] is not None else datetime.today().strftime(
    ...
    def assign_PIC(row, status='status'):
        ...
        # TODO: add criteria no future service appointments for planning and scheduling PIC. remove PM criteria.
        #  Add statuses: 'Reschedule', 'Pricing/Material Review Needed', 'Scheduled', 'Scheduling in Progress'
    ...
    product_transfers = pd.DataFrame()
    service_appointments = pd.DataFrame()

    # TODO: Check if async call possible here
    # TODO: wo-details: schedStart vs arrivalWindowStartTime
    for i in range(iterations):
        chunk = next(get_chunk)
        ...
        # Get & combine Product transfers with chunk_product_transfer
        chunk_product_transfer = attempt_function(cmms.get_product_transfers_by_work_order_ids,
                                                  chunk['workOrderId'].tolist())
        product_transfers = pd.concat([product_transfers, chunk_product_transfer])
        # get & combined service appointment
        chunk_service_appointments = attempt_function(cmms.get_service_appointments_by_work_order_ids,
                                                      chunk['workOrderId'].tolist())

        service_appointments = pd.concat([service_appointments, chunk_service_appointments])

    # Service appointment date.
    service_appointments.rename(
        columns={'arrivalWindowStartTime': 'Service Appointments Date', 'status': 'SA_status'}, inplace=True
    )
    service_appointments = service_appointments[
        ['workOrderId', 'Service Appointments Date', 'appointmentNumber', 'SA_status']
    ]
    service_appointments['Service Appointments Date'] = pd.to_datetime(
        service_appointments['Service Appointments Date'], errors='coerce', utc=True
    )
    service_appointments = service_appointments[
        service_appointments['Service Appointments Date'] >= datetime.now(pytz.utc).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
    ]
    service_appointments = service_appointments[(pd.notna(service_appointments['Service Appointments Date']))]
    service_appointments = service_appointments[service_appointments['Service Appointments Date'] != 'None']
    service_appointments.sort_values(
        by=['workOrderId', 'Service Appointments Date', 'appointmentNumber'],
        ascending=[True, True, True], inplace=True
    )
    service_appointments.drop_duplicates(subset="workOrderId", inplace=True)
    # service_appointments['duplicate'] = service_appointments.duplicated(subset='workOrderId', keep='first')
    # service_appointments = service_appointments[service_appointments['duplicate'] is False]
    service_appointments['Service Appointments Date'] = service_appointments['Service Appointments Date'].apply(
        lambda x: x.strftime('%Y-%m-%d') if pd.notnull(x) else None
    )
    service_appointments.reset_index(inplace=True, drop=True)
    work_orders = pd.merge(
        work_orders, service_appointments, left_on='workOrderId', right_on='workOrderId', how='left'
    )

    # Product transfer to merge with work_orders
    product_transfers.rename(
        columns={'workOrderId': 'PT_workOrderId', 'productTransferNumber': 'PT_Number',
                 'status': 'PT_status', 'poNumber': 'PT_poNumber'}, inplace=True)
    product_transfers = product_transfers[['PT_workOrderId', 'PT_Number', 'PT_status']]
    product_transfers_groupby_workOrderId = product_transfers.groupby('PT_workOrderId').agg(
        lambda x: ', '.join(map(str, x))
    ).reset_index()
    work_orders = pd.merge(work_orders, product_transfers_groupby_workOrderId, left_on='workOrderId',
                           right_on='PT_workOrderId', how='left')
    ...
    work_orders['days_in_current_status_bins_30'] = pd.cut(work_orders['days_in_current_status_30'],
                                                           ...
                                                           labels=['a. <=30', 'b. 30-90', 'c. 90-180', 'd. 180-500'],
                                                           include_lowest=True, precision=0)
    work_orders = pd.merge(work_orders, fleet_metadata, how='left', left_on='siteId', right_on='SCADA_GUID')
    ...
    # TODO add poNumber if empty, PT or PR
...
def run_prioritization_report(email, position, environment=FPP_ENVIRONMENT.value, cc_list=None):
    ...
    if position in ['Supply_Chain', 'fleet', 'north_america']:
        fleet = Fleet(filter_="('In Operation', 'Signed')", filter_column='OnM_Project_Status', filter_operator='IN')
    else:
        fleet = Fleet(email, position)

    fleet_metadata = fleet.get_summary_dataframe(
        columns=['AssetTitle', 'SCADA_GUID', 'Customer_Name', 'OnM_Regional_Manager', 'OnM_Area_Manager',
                 'OnM_Site_Manager', 'Site Region']).rename(
    ...
    if len(work_orders) == 0:
        body = "There are no work orders to prioritize."
    ...
    if len(work_orders[work_orders['is_active'] == 1]) == 0:
        body = "There are no active work orders to prioritize."
    ...
    work_orders['PR_numbers'] = work_orders.apply(
        lambda x: f"{x['PR_numbers']},{x['poNumber']}"
        if str(x['poNumber']).startswith('PR') and
        str(x['poNumber']).replace('-', '').lower() not in
        str(x['PR_numbers']).replace('-', '').lower()
        else x['PR_numbers'],
        axis=1
    )
    work_orders['purchase_order_numbers'] = work_orders.apply(
        lambda x: f"{x['purchase_order_numbers']},{x['poNumber']}"
        if str(x['poNumber']).startswith('PO') and
        str(x['poNumber']).replace('-', '').lower() not in
        str(x['purchase_order_numbers']).replace('-', '').lower()
        else x['purchase_order_numbers'], axis=1
    )

    details = work_orders[work_orders['is_active'] == 1][
        [
            'plantName', 'workOrderNumber', 'status', 'subStatus', 'description', 'assetDescription', 'PIC',
            'workTypeCategory', 'workType', 'wopriority', 'capacityOffline', 'lost_MWh', 'createdDate',
            'Service Appointments Date', 'target_complete_date', 'days_in_current_status', 'escalation_name',
            'PT_Number', 'PT_status', 'PR_numbers', 'PR_statuses', 'purchase_order_numbers', 'warranty_numbers',
            'Region Manager', 'Area Manager', 'Site Supervisor', 'Customer_Name', 'approx_lost_mwh', 'approx_offline_kw'
        ]
    ].rename(columns={
        'capacityOffline': 'offline_kW',
        'subStatus': 'substatus',
        'Service Appointments Date': 'schedStart'
    })
    details['createdDate'] = details.apply(lambda x: x['createdDate'][:10], axis=1)
    details['createdDate'] = pd.to_datetime(details['createdDate']).dt.date
    # details['schedStart'] = details.apply(
    #     lambda x: x['schedStart'][:10] if x['schedStart'] is not None else None, axis=1)
    details['schedStart'] = pd.to_datetime(details['schedStart']).dt.date
    details['target_complete_date'] = details.apply(
        lambda x: x['target_complete_date'].strftime('%Y/%m/%d %H:%M'), axis=1)
    details['target_complete_date'] = pd.to_datetime(details['target_complete_date']).dt.date
    ...
    try:
        ...
        writer.sheets['Work Orders'].autofilter(0, 0, len(details.index)-1, len(details.columns)-1)
        writer.sheets['Work Orders'].set_column(0, 30, 25)

        # prioritization_report set excel date format
        date_format = workbook.add_format({"num_format": "mm-dd-yyyy"})
        writer.sheets['Work Orders'].set_column('M:M', None, date_format)
        writer.sheets['Work Orders'].set_column('N:N', None, date_format)
        writer.sheets['Work Orders'].set_column('O:O', None, date_format)
...
def run_escalation_report(email, position, environment=FPP_ENVIRONMENT.value, cc_list=None):
    ...
    fleet_metadata = fleet.get_summary_dataframe(
        columns=['AssetTitle', 'SCADA_GUID', 'Customer_Name', 'OnM_Regional_Manager', 'OnM_Area_Manager', 'OnM_Site_Manager',
                 'Site Region']).rename(
        columns={'OnM_Regional_Manager': 'Region Manager', 'OnM_Area_Manager': 'Area Manager',
                 'OnM_Site_Manager': 'Site Supervisor'})
    sites = fleet.site_asset_titles
    ...
    active_fleet = Fleet(filter_="('In Operation', 'Signed')", filter_column='OnM_Project_Status', filter_operator='IN')
    ...
    details = work_orders[work_orders['is_active'] == 1][
        [
            ...
            'workType', 'wopriority', 'capacityOffline', 'lost_MWh', 'createdDate', 'schedStart',
            'target_complete_date', 'days_in_current_status', 'escalation_name', 'PT_Number', 'PT_status', 'PR_numbers',
            'PR_statuses', 'purchase_order_numbers', 'warranty_numbers', 'Region Manager', 'Area Manager',
            'Site Supervisor', 'Customer_Name'
        ]
    ].rename(columns={'capacityOffline': 'offline_kW'})
    details['createdDate'] = details.apply(lambda x: x['createdDate'][:10], axis=1)
    details['createdDate'] = pd.to_datetime(details['createdDate']).dt.date
    details['schedStart'] = details.apply(
        lambda x: x['schedStart'][:10] if x['schedStart'] is not None else None, axis=1
    ...
    details['schedStart'] = pd.to_datetime(details['schedStart']).dt.date
    details['target_complete_date'] = details.apply(lambda x: x['target_complete_date'].strftime('%m-%d-%Y'), axis=1)
    details['target_complete_date'] = pd.to_datetime(details['target_complete_date']).dt.date
...
def run_work_order_progress_report(email, position, fleet_future_cms, environment=FPP_ENVIRONMENT.value, cc_list=None):
    ...
    fleet_metadata = fleet.get_summary_dataframe(
        columns=['AssetTitle', 'SCADA_GUID', 'Customer_Name', 'OnM_Regional_Manager', 'OnM_Area_Manager', 'OnM_Site_Manager',
                 'Site Region', 'Capacity_DC', 'Commission_Date']).rename(
        columns={'OnM_Regional_Manager': 'Region Manager', 'OnM_Area_Manager': 'Area Manager',
    ...
    future_cms_by_site = pd.merge(future_cms_by_site, fleet_metadata[['AssetTitle', 'SCADA_GUID', 'mwdc', 'commission_date']],
                                  how='left', left_on='siteId', right_on='SCADA_GUID')
    ...
    reporting_columns = [
        'plantName', 'workOrderNumber', 'status', 'description', 'assetDescription', 'PIC', 'workTypeCategory',
        'workType', 'wopriority', 'capacityOffline', 'dateReceived', 'completedDate', 'target_complete_date',
        'escalation_name', 'PT_Number', 'PT_status', 'PR_numbers', 'PR_statuses', 'purchase_order_numbers',
        'warranty_numbers'
    ]
    ...
    active_work_orders['target_complete_date'] = active_work_orders[
        'target_complete_date'].dt.date
    active_work_orders['dateReceived'] = active_work_orders['dateReceived'].str[:10]
    active_work_orders['dateReceived'] = pd.to_datetime(active_work_orders['dateReceived']).dt.date
    active_work_orders['completedDate'] = active_work_orders['completedDate'].dt.date
...
def calculate_fleet_cm_forecast():
    ...
    fleet_metadata = fleet.get_summary_dataframe(
        columns=['AssetTitle', 'Customer_Name', 'OnM_Regional_Manager', 'OnM_Area_Manager', 'OnM_Site_Manager',
                 'Site Region', 'Capacity_DC']).rename(
        columns={'OnM_Regional_Manager': 'Region Manager', 'OnM_Area_Manager': 'Area Manager',
...
def run_supply_chain_prioritization_report(email, environment=FPP_ENVIRONMENT.value, cc_list=None):
    ...
    for i in range(iterations):
        ...
        # Get & combined service appointment with chunk_service_appointments
        chunk_service_appointments = attempt_function(cmms.get_service_appointments_by_work_order_ids,
                                                      chunk['workOrderId'].tolist())
        service_appointments = pd.concat([service_appointments, chunk_service_appointments])
    ...
    # Service appointment date.
    service_appointments.rename(
        columns={'arrivalWindowStartTime': 'Service Appointments Date', 'status': 'SA_status'},
        inplace=True)
    service_appointments['duplicate'] = service_appointments.duplicated(subset='workOrderId', keep='first')
    service_appointments = service_appointments[
        (service_appointments['duplicate'] == 'False') & (
            pd.notna(service_appointments['Service Appointments Date']))]
    service_appointments = service_appointments[
        'workOrderId', 'Service Appointments Date', 'appointmentNumber', 'SA_status']

    work_orders = pd.merge(work_orders, service_appointments, left_on='workOrderId', right_on='workOrderId',
                           how='left')

    # Product transfer to merge with work_orders
    product_transfers.rename(
        columns={'workOrderId': 'PT_workOrderId', 'productRequestNumber': 'PT_Number',
                 'status': 'PT_status', 'poNumber': 'PT_poNumber'}, inplace=True)
    product_transfers = product_transfers[['PT_workOrderId', 'PT_Number', 'PT_status']]
    product_transfers_groupby_workOrderId = product_transfers.groupby('PT_workOrderId').agg(
        lambda x: ', '.join(map(str, x))
    ...
    product_request_reporting_columns = [
        'productRequestNumber', 'poNumber', 'recordTypeName', 'status_pr', 'vendor', 'orderDescription',
        'workOrderNumber', 'plantName', 'workOrderNumber', 'target_complete_date', 'status', 'description',
        'assetDescription', 'PIC', 'workTypeCategory', 'workType', 'wopriority', 'capacityOffline', 'lost_MWh',
        'createdDate', 'schedStart', 'days_in_current_status', 'escalation_name', 'PT_Number', 'PT_status',
        'PR_numbers', 'PR_statuses', 'warranty_numbers', 'Region Manager', 'Area Manager', 'Site Supervisor',
        'Customer_Name'
    ]
...
if __name__ == '__main__':
    ...
    environment = Environment.QA.value
    pools = 20
    ...
    # TODO: Add robb.wilson to cc
    emails.append('douglas.taylor@novasourcepower.com'), positions.append('north_america'), environments.append(
        environment), ccs.append([])
    ...
    # run escalation subscriptions
    data_driven_reports = ['OnM_Regional_Manager', 'OnM_Area_Manager', 'OnM_Site_Manager']
    ...
    emails.append(
        'akshay.sagar@novasourcepower.com'), positions.append('CEO'), environments.append(environment), ccs.append(
        'frank.kelly@novasourcepower.com')
    emails.append('douglas.taylor@novasourcepower.com'), positions.append('President'), environments.append(
        environment), ccs.append('frank.kelly@novasourcepower.com')
    emails.append('UPP-Planning_Scheduling@novasourcepower.com'), positions.append(
        'Planning_Scheduling'), environments.append(environment), ccs.append([])
    emails.append('account.management@novasourcepower.com'), positions.append(
        'Account_Management'), environments.append(environment), ccs.append(
        ['kirby.hunt@novasourcepower.com', 'benjamin.pochop@novasourcepower.com',
         'kristopher.arndt@novasourcepower.com'])
    tasks = [*zip(emails, positions, environments, ccs)]

    ctx2 = mp.get_context("spawn")
    with ctx2.Pool(pools) as pool:
        try:
            smtp_response = pool.starmap(run_escalation_report, tasks)
        except:
            logger.exception(f'escalation report failed for {tasks}')
            pass

    # run work order progress subscriptions (mondays only)
    if datetime.today().weekday() == 0:  # 0:
